...
import serial
from prometheus_client import Gauge, Histogram, Enum, start_http_server


try:
    __version__ = version("tarpn-bdd")
...
def main():
    ...
    parser.add_argument("config", nargs="?", default="config/bdd.ini", help="Config file")
    ...
    if not os.path.exists(args.config) and os.path.basename(args.config) == "bdd.ini":
        shutil.copyfile("config/bdd.ini.sample", args.config)
    ...
    logger.info("Starting Prometheus server on port 8000")
